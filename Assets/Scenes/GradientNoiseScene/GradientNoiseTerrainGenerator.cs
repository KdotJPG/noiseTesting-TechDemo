...
public class GradientNoiseTerrainGenerator : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (randomize) {
            ...
            noise02Seed = Random.Range(int.MinValue, int.MaxValue);
    ...
    float getNoise(long seed, double xCoord, double yCoord) {
        float value;
        if (is3DNoiseModeEnabled)
            value = OpenSimplex2.Noise3_ImproveXY(seed, xCoord, yCoord, scrollOffset);
        else
            value = OpenSimplex2.Noise2(seed, xCoord + scrollOffset, yCoord + scrollOffset);
